...
https://youtu.be/IfG-zEkk9KQ

Walkthrough generated by `scripts/demo.sh`: Trust Gate blocks a poisoned external Agent Card → Trust Gate allows a signed registered card → Release Gate blocks a Vendor Exception Agent that approved a $50K exception without policy lookup → Release Gate passes the remediated agent → revoke a shipped release after a production regression (signed, tamper-evident) → audit trail in the Verdict Inspector.
...
- **Idempotent persistence + revocation API.** Re-submitting the same `release_id` returns the existing record. `agentgov release revoke <id>` records revoke metadata (`revoked_at`, `revoked_by`, `revoke_reason`) on the existing row via a targeted `UPDATE` — `payload_json` and the original HMAC `signature` are never touched, so the original verdict stays independently verifiable. The revocation itself is separately signed (`revoke_signature`), so the revoke metadata is tamper-evident too.
- **Hardened against adversarial input.** Trust and release gates were pressure-tested with crafted attacks and fixed: malformed Agent Cards fail closed instead of crashing, prompt injection is scanned across every attacker-controlled field (with Unicode normalization), the trust registry matches by hostname rather than substring, a skill-allowlist violation is a hard BLOCK, and `persist_decision` refuses any decision whose signature does not verify. 35 unit + smoke tests.
