...
- **Idempotent persistence + revocation API.** Re-submitting the same `release_id` returns the same record. `agentgov release revoke <id>` appends an audit row without rewriting history. The revoke field set (`revoked_at`, `revoked_by`, `revoke_reason`) is signed alongside the original verdict.
